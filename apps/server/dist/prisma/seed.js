...
const hasCounterSuffix = (shortName) => /-\d+$/.test(shortName);
const buildUniqueShortName = (baseShortName, usedShortNames) => {
    if (!usedShortNames.has(baseShortName))
...
async function main() {
    ...
    const dedupedFromSource = new Map();
    const allUnisRaw = [
    ...
    const groupedByNormalizedName = new Map();
    ...
    const existingNormalizedNames = new Set(currentUniversities.map((uni) => normalizeUniversityName(uni.name)));
