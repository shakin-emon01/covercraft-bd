...
const login = async (req, res) => {
    try {
        ...
        try {
            await auth_service_1.AUTH_SERVICE.upsertSession(user.id, ipAddress, userAgent);
        }
        catch (sessionError) {
            console.error('Session creation failed after login:', sessionError);
        }
        const accessToken = generateToken(user.id, user.role);
        const refreshToken = await auth_service_1.AUTH_SERVICE.createRefreshToken(user.id);
        return res.json({
            token: accessToken,
            accessToken,
            refreshToken,
    ...
    try {
        ...
        if (!user) {
            ...
            if (Object.keys(updateData).length > 0) {
                user = await prisma_1.default.user.update({ where: { id: user.id }, data: updateData });
            }
        }
        if (user.status === 'SUSPENDED')
            return res.status(403).json({ message: 'Your account is suspended. Please contact support.' });
