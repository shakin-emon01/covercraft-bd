...
export const createAdminReview = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
...
export const updateAdminReview = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (req.body?.rating !== undefined) {
      const rating = parseRating(req.body.rating);
      if (!rating) return res.status(400).json({ message: 'Rating must be an integer between 1 and 5.' });
      patch.rating = rating;
    }
